    ...
    class Natural
    {
       ...
       // Выполнил Плотников А.А.
        ...
        public static Natural SUB_NN_N(Natural a, Natural b)
        {
            ...
            bool flag = false; // flag - флажок займа десятка, zero - проверка на нулевое число
            int i, temp, zero = 0;
            for (i = 0; i < d.n; i++) // вычитаем до конца второго числа
            {
                ...
                if (c.digits[i] == 0) zero += 1; // считаем нули в числа
            }
            if (zero == c.n) return c = new Natural(); // если в числе количество нулей совпадает с количеством цифр, то выводим ноль
            if (flag)
            {
                for (i = d.n; i < c.n; i++) // если мы заняли десяток, а вычитать уже нечего, то ищем разряд, у которого его можно занять
                {
                    if (c.digits[i] > 0) // если нашли
                    {
                        c.digits[i] -= 1; // то отнимаем
                        if (i == (c.n - 1) && c.digits[i] == 0) // если это была единица в самом старшем разряде
                        {
                            c.n--; // то сокращаем это число на один разряд
                            c.digits.RemoveAt(i); // и удаляем этот самый разряд
                        }
                        break;
                    }
                    else c.digits[i] = 9; //иначе всовываем девятку
                }
            }
            if (zero > 0) // если в ответе есть нули
                for (i = (c.n - 1); true; i--) // то проверям не заканчивается ли число ими
                {
                    if (c.digits[i] == 0) // если находим ноль
                    {
                        c.n--; // то сокращаем это число на один разряд
                        c.digits.RemoveAt(i); // и удаляем этот самый разряд
                    }
