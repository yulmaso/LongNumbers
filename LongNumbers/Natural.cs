    ...
    class Natural
    {
       ...
       // Выполнил Плотников А.А.
        ...
        public static Natural SUB_NN_N(Natural a, Natural b)
        {
            ...
            {
                bool k = true; // флажок займа десятка
                int i; 
                for (i = 0; i < d.n; i++) // вычитаем до окончания разрядов меньшего числа
                {
                    if (k) // 1) если в этом разряде мы десяток не занимали
                    {
                        if (c.digits[i] < d.digits[i]) // если десяток нужно занять
                        {
                            c.digits[i] += 10; // занимаем
                            k = false;
                        }
                        c.digits[i] -= d.digits[i]; // собственно вычитаем
                    }
                    else // 2) если мы заняли десяток в этом разряде
                    {
                        if (c.digits[i] < d.digits[i] + 1) // если и в следующем разряде нужно занять десяток
                        {
                            c.digits[i] += 10; // занимаем
                            c.digits[i] -= 1; // отнимаем занятый младшим разрядом десяток
                        }
                        else
                        {
                            k = true; // если занимать в следующем разряде не нужно
                            c.digits[i] -= 1; // отнимаем занятый младшим разрядом десяток
                        }
                        c.digits[i] -= d.digits[i]; // собственно вычитаем
                    }
